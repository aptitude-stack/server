# Aptitude Scope: Repository vs Resolver

## Purpose

This document defines the hard boundary between `aptitude-repository` and `aptitude-resolver` so responsibilities do not overlap.
...
| Capability | Repository Owner | Resolver Owner | Notes |
| --- | --- | --- | --- |
...
2. Resolver normalizes request and asks repository for deterministic resolution.
...
6. Resolver never writes directly to repository DB; all interaction is API-based.
...
- Deterministic package selection rules live in repository policy; resolver-specific filtering must be additive and explicit.

## Non-Goals

- Building a monolith that combines repository persistence and resolver runtime in one deployable.
